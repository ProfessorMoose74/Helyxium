...
class ChildProfile:
    ...
    def __post_init__(self):
        if self.allowed_data_collection is None:
            self.allowed_data_collection = [DataCollectionType.NECESSARY_OPERATION.value]
...
class COPPAManager:
    ...
    def create_child_profile(self, user_id: str, age: int) -> bool:
        ...
        try:
            if age >= 13:
                self.logger.warning(f"Attempted to create child profile for user >= 13: {age}")
                return False
    ...
    def request_parental_consent(self, user_id: str, parent_email: str, 
                               parent_name: str, method: ParentalConsentMethod) -> Optional[str]:
        """
    ...
    def verify_parental_consent(self, verification_token: str, 
                              consent_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            self.security_logger.coppa_verification(child_profile.age if child_profile else 0, True)
    ...
    def is_data_collection_allowed(self, user_id: str, 
    ...
    def update_parental_settings(self, user_id: str, settings: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "session_time_limits" in settings:
                child_profile.session_time_limits.update(settings["session_time_limits"])
            
            if "content_filters" in settings:
                child_profile.content_filters = settings["content_filters"]
            
            if "social_restrictions" in settings:
                child_profile.social_restrictions.update(settings["social_restrictions"])
            
            if "break_reminders_enabled" in settings:
                child_profile.break_reminders_enabled = settings["break_reminders_enabled"]
    ...
    def _verify_consent_method(self, consent: ParentalConsent, 
                             consent_data: Dict[str, Any]) -> bool:
    ...
    def _send_consent_request(self, consent: ParentalConsent):
        ...
        self.logger.info(f"Consent request sent to {consent.parent_email} for {consent.child_user_id}")
    ...
    def cleanup_expired_consents(self):
        ...
        current_time = time.time()
        expired_tokens = [
            token for token, consent in self._consent_records.items()
            if consent.expires_at and current_time > consent.expires_at and not consent.is_verified
        ]
        ...
        if expired_tokens:
            self._save_coppa_data()
            self.logger.info(f"Cleaned up {len(expired_tokens)} expired consent requests")
