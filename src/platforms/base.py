...
class BasePlatformConnector(QObject, ABC):
    ...
    error_occurred = pyqtSignal(
        str, str, str
    )  # platform_name, error_type, error_message
    ...
    async def send_message(
        self, recipient_id: str, content: str, message_type: str = "text"
    ) -> bool:
        """
    ...
    async def discover_worlds(
        self, search_query: Optional[str] = None, limit: int = 20
    ) -> List[PlatformWorld]:
        """
    ...
    def _set_status(
        self, status: ConnectionStatus, error_message: Optional[str] = None
    ):
        ...
        if old_status != status:
            ...
            if error_message:
                ...
                self.error_occurred.emit(
                    self.platform_name, "connection", error_message
                )
    ...
    def get_platform_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_capabilities": [
                cap.value for cap in self.get_supported_capabilities()
            ],
            "current_user": (
                {
