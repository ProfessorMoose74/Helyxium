    ...
    def _detect_via_openvr(self) -> None:
        ...
        try:
            ...
            device = VRDevice(
                ...
                is_connected=True,
                capabilities=self.DEVICE_CAPABILITIES.get(device_type, VRCapabilities()),
            ...
            for device in devices:
                ...
                if device_id in self.USB_DEVICE_IDS:
                    ...
                    try:
                        ...
                        product = usb.util.get_string(device, device.iProduct) or "Unknown VR Headset"
                        serial = None
                        try:
                    ...
                    # Avoid duplicates
                    if not any(d.serial_number == serial and d.device_type == device_type 
    ...
    def _detect_windows_vr(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["powershell", "-Command", "Get-PnpDevice | Where-Object {$_.FriendlyName -like '*Mixed Reality*' -or $_.FriendlyName -like '*HoloLens*'}"],
                capture_output=True,
                text=True,
                timeout=10
            )
            ...
            if result.returncode == 0:
                ...
                for pattern, device_type in vr_patterns:
                    if re.search(pattern, lines):
                        ...
                        # Avoid duplicates
                        if not any(d.device_type == device_type for d in self._detected_devices):
                            self._detected_devices.append(device)
                ...
                for process, device_type in process_patterns.items():
                    if process in output:
                        ...
                        if not any(d.device_type == device_type for d in self._detected_devices):
                            device = VRDevice(
                                device_type=device_type,
                                name=f"{device_type.value.replace('_', ' ').title()} (Process Detection)",
                                manufacturer="Unknown",
    ...
    def _compile_detection_results(self) -> Dict[str, Any]:
        ...
        if self._detected_devices:
            result["capabilities_summary"] = {
                "best_resolution": max(
                    (dev.capabilities.resolution_per_eye[0] * dev.capabilities.resolution_per_eye[1] 
                     for dev in self._detected_devices),
                    default=0
                ),
                "highest_refresh_rate": max(
                    (dev.capabilities.max_refresh_rate for dev in self._detected_devices),
                    default=0
                ),
                "widest_fov": max(
                ...
                "has_any_hand_tracking": any(dev.capabilities.has_hand_tracking for dev in self._detected_devices),
                "has_any_eye_tracking": any(dev.capabilities.has_eye_tracking for dev in self._detected_devices),
                "has_any_passthrough": any(dev.capabilities.has_passthrough for dev in self._detected_devices),
                "has_any_wireless": any(dev.capabilities.has_wireless for dev in self._detected_devices)
            }
