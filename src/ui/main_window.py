...
class WelcomeWidget(QWidget):
    ...
    def __init__(
        self, localization_manager: LocalizationManager, theme_manager: ThemeManager
    ):
        super().__init__()
    ...
    def _setup_ui(self):
        ...
        if hasattr(self.logo_widget, "animation_finished"):
            self.logo_widget.animation_finished.connect(
                self._on_logo_animation_finished
            )
...
class DashboardWidget(QWidget):
    ...
    def update_platforms(self, platforms: Dict[str, Any]):
        ...
        for platform, info in platforms.items():
            status = (
                tr("messages.connected")
                if info.get("connected")
                else tr("messages.disconnected")
            )
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        self, theme_manager: ThemeManager, localization_manager: LocalizationManager
    ):
        super().__init__()
    ...
    def _setup_ui(self):
        ...
        self.welcome_widget = WelcomeWidget(
            self.localization_manager, self.theme_manager
        )
        ...
        self.main_tabs.addTab(
            self.dashboard_widget, tr("ui.main_window.tabs.dashboard")
        )

        # Platforms tab
        self.platforms_widget = PlatformsWidget(self.localization_manager)
        self.main_tabs.addTab(
            self.platforms_widget, tr("ui.main_window.tabs.platforms")
        )
    ...
    def _check_welcome_visibility(self):
        ...
        if (
            self.welcome_widget
            and not self.welcome_widget.isVisible()
            and self._is_welcome_visible
        ):
