...
class HelyxiumInstaller:
    ...
    def print_header(self):
        ...
        print(
            f"{Colors.BOLD}{Colors.HEADER}    Helyxium Universal VR Bridge Platform{Colors.END}"
        )
        print(
            f"{Colors.BOLD}{Colors.BLUE}           Installation Wizard v{INSTALLER_VERSION}{Colors.END}"
        )
        print(f"{Colors.CYAN}{'='*60}{Colors.END}\n")

        print(
            f"{Colors.BLUE}Target Platform:{Colors.END} {self.system.title()} ({self.architecture})"
        )
        print(
            f"{Colors.BLUE}Python Version:{Colors.END} {'.'.join(map(str, self.python_version[:3]))}"
        )
    ...
    def check_python_version(self) -> bool:
        ...
        if self.python_version < PYTHON_MIN_VERSION:
            print(
                f"{Colors.RED}❌ Python {'.'.join(map(str, PYTHON_MIN_VERSION))} or higher required.{Colors.END}"
            )
            print(f"   Current version: {'.'.join(map(str, self.python_version[:3]))}")
            return False
        elif self.python_version >= PYTHON_MAX_VERSION:
            print(
                f"{Colors.YELLOW}⚠️  Python {'.'.join(map(str, self.python_version[:3]))} is newer than tested versions.{Colors.END}"
            )
            print(
                f"   Helyxium supports up to Python {'.'.join(map(str, PYTHON_MAX_VERSION))}"
            )
    ...
    def check_system_requirements(self) -> bool:
        ...
        try:
            if self.system == "windows":
                ...
                stat = shutil.disk_usage(
                    ...
                    if self.install_dir.parent.exists()
                    else Path.home()
                )
            ...
            if free_space < required_space:
                print(
                    f"{Colors.RED}❌ Insufficient disk space. Required: 500MB, Available: {free_space // (1024*1024)}MB{Colors.END}"
                )
                return False

        except Exception:
            print(
                f"{Colors.YELLOW}⚠️  Could not check disk space, continuing...{Colors.END}"
            )
        ...
        if missing_libs:
            print(
                f"{Colors.YELLOW}⚠️  Missing system libraries: {', '.join(missing_libs)}{Colors.END}"
            )
            if self.system == "linux":
                print(
                    "   Try installing: sudo apt-get install libgl1-mesa-glx libx11-6 libxcb1"
                )
    ...
    def create_directories(self) -> bool:
        ...
        try:
            ...
            print(
                f"{Colors.RED}❌ Permission denied. Try running as administrator/sudo.{Colors.END}"
            )
            return False
    ...
    def download_helyxium(self) -> bool:
        ...
        if (current_dir / "main.py").exists() and (current_dir / "src").exists():
            ...
            try:
                ...
                for item in current_dir.iterdir():
                    ...
                    if item.is_dir():
                        shutil.copytree(
                            item, self.app_dir / item.name, dirs_exist_ok=True
                        )
                    else:
        ...
        try:
            ...
            extracted_dirs = [
                ...
                if d.is_dir() and d.name.startswith("helyxium")
            ]
    ...
    def create_virtual_environment(self) -> bool:
        ...
        try:
            ...
            print(
                f"{Colors.RED}❌ Failed to create virtual environment: {e}{Colors.END}"
            )
            return False
...
"""
    ...
    def create_desktop_shortcuts(self) -> bool:
        ...
        try:
            ...
            print(
                f"{Colors.YELLOW}⚠️  Desktop integration failed (optional): {e}{Colors.END}"
            )
    ...
    def _create_windows_shortcuts(self):
        ...
        try:
            ...
            shortcut.IconLocation = str(
                self.app_dir / "assets" / "icons" / "helyxium.ico"
            )
...
"""
            ...
            if result.returncode == 0:
                ...
                print(
                    f"{Colors.YELLOW}⚠️  Installation test warnings: {result.stdout}{Colors.END}"
                )
                return True  # Non-critical
    ...
    def install(self) -> bool:
        ...
        print(
            f"{Colors.BOLD}{Colors.GREEN}    🎉 Helyxium Installation Complete! 🎉{Colors.END}"
        )
